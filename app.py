    ...
    .subtitle{font-size:18px;color:#dbeafe;}
    .header-box{
    ...
    .metric-value{font-size:34px;color:#1e40af;font-weight:bold;}
    .ai-box{
    ...
    <div class="big-title">📊 Executive Insights AI</div>
    <div class="subtitle">AI Powered Business Intelligence Agent for Monday.com</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        for board_name, board_id in board_dict.items():
            ...
            try:
                ...
                result.append({"Board": board_name, "Items": len(items)})
                board_data += f"\n===== {board_name} =====\n"
                ...
                for item in items:
                    ...
                    for col in item["column_values"]:
                        ...
                        value = col["text"] or "Not Available"
        ...
        with c1:
            st.metric("📋 Boards", len(board_dict))
        with c2:
            st.metric("📦 Total Items", total_items)
        with c3:
            st.metric("💼 Work Orders", df.loc[df.Board=="Work Orders","Items"].sum())
        with c4:
            st.metric("💰 Deals", df.loc[df.Board=="Deals","Items"].sum())
